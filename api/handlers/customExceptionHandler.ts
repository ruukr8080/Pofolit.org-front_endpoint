...
export const ex = (error: AxiosError) => {
  ...
  if (isApiResult(responseData) && responseData.error) {
    switch (responseData.error) {
      case AppErrorCode.DUPLICATE_SOCIAL_EMAIL:
        alert("이미 다른 소셜 계정으로 가입된 이메일입니다.");
        break;
      case AppErrorCode.DUPLICATE_USER:
        alert("이미 존재하는 유저입니다.");
        break;
      case AppErrorCode.DUPLICATE_NICKNAME:
        alert("이미 사용 중인 닉네임입니다.");
        break;
      case AppErrorCode.NOT_FOUND_USER:
      ...
      case AppErrorCode.INVALID_TOKEN:
        ...
        break;
      case AppErrorCode.EXPIRED_TOKEN:
        alert(
        ...
        break;
      case AppErrorCode.INVALID_TYPE:
      case AppErrorCode.INVALID_VALUE:
        alert("요청한 파라미터의 타입이나 값이 올바르지 않습니다.");
        break;
      case AppErrorCode.EXCEPTION_CONSTRUCTOR:
